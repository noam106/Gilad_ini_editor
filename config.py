...
class Config:
    ...
    def set_accounts(self, new_accounts: dict):
        self.ib['accounts'] = new_accounts

    def set_main_contract(self, new_main_contract: list) -> bool:
        main_contract_tuple = ('UVXY', 'VXX', 'VIXY', 'SVIX', 'UVIX', 'SVXY')
        for i in new_main_contract:
            if i not in main_contract_tuple:
                return False
            else:
                self.policy['main_contract'] = new_main_contract
                return True

    def set_contract_levregy(self, new_contract_levregy: float) -> bool:
        self.policy['contract_leverage'] = new_contract_levregy
        return True

    def set_contract_type(self, new_contract_type: str) -> bool:
        contract_type_tuple = ('regular', 'inverse')
        if new_contract_type in contract_type_tuple:
            self.policy['contract_type'] = new_contract_type
            return True
        else:
            return False
